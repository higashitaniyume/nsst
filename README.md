...
页面最顶上是一条**客户端信息栏**，说明这条会话是谁、从哪儿连过来的。它一半来自服务端在这次请求里看到的东西，一半来自浏览器自己的 API：

| 分组 | 字段 | 来源 |
| --- | --- | --- |
| 连接 | IP 地址、地址来源、连接对端 | 服务端（见 [`GET /api/client`](#get-apiclient)） |
| 连接 | 传输（HTTP/1.1 还是 HTTP/2、明文还是 TLS） | 服务端 |
| 连接 | 服务端主机 | 服务端（请求的 `Host`） |
| 客户端 | 浏览器、语言、时区、屏幕、设备、网络 | 浏览器 `Navigator` API |
| 客户端 | 时钟偏差 | 服务端 `server_time` 减去本机时钟 |
| 客户端 | User-Agent、页面地址 | 只在导出的元信息里出现 |

**地址来源**会明说是直连，还是来自某个代理请求头（`X-Forwarded-For`、`CF-Connecting-IP`、`X-Real-IP`、`True-Client-IP`）。这不是装饰：没有前置代理时这些请求头谁都能伪造，所以界面把「地址从哪来」和地址本身一起显示，而不是假装它可信。有代理链时鼠标悬停在来源上能看到完整跳数。

点右上角的「隐藏」把这一栏收成一行，选择记在 `localStorage`，下次打开保持收起。窄屏下字段自动从每行 5 个减到 2 个，不出现横向滚动。

浏览器的那些字段都在本机计算，不会发到任何地方；服务端也不做任何地理定位或 IP 归属查询。时钟偏差跨一次请求测得，误差约为往返时间的一半。

### 导出表格

两种导出，都是 **UTF-8 CSV**（带 BOM，Excel 双击不乱码）：

| 按钮 | 位置 | 内容 |
| --- | --- | --- |
| 导出 | 每张协议卡片的右上角 | 这个协议的两张表 + 判定 + 逐帧明细 |
| 导出总表格 | 页面最右上角 | 上一次运行涉及的所有协议，并排成一张表 + 各协议的逐帧明细 |

导出的是**屏幕上那些读数本身**，不是另算一遍：表里显示的字符串就是文件里的字符串，语言也跟着界面走（中文界面导出中文表头）。

文件里先是元信息（导出时间、客户端 IP 与来源、浏览器、测试参数、预期帧数、运行状态、逐帧明细的保留范围），然后是汇总表本体，列固定为 `分组, 指标, <协议列…>`：

- 总表格里每个协议占一列，行按「连接与流量 → 时延与间隔 → 判定」排列，横向对比不用来回翻。
- **只导出最近一次运行里真正跑过的协议**：这一轮只勾了 HTTP 流，总表格就只有一列，不会把上一轮 SSE 的旧读数混进来。
- 单协议的文件和总表格里的那一列完全一致，列数、行数、字段名都一样。
- 文件名形如 `nsst-all-20260211-093000.csv`、`nsst-sse-20260211-093000.csv`，时间戳是本地时间。
- 不含逗号、引号、换行的字段不加引号；含有的按 RFC 4180 加引号、内部引号翻倍（User-Agent 里就有逗号）。
- 每一次导出都会在事件日志里留一条记录，包含文件名。

#### 逐帧明细

每份导出在汇总表之后还带一张**逐帧明细表**，一帧一行。汇总表说的是这一轮「平均怎么样」，明细表是它背后的原始序列——想画间隔分布、想找出那次迟到到底卡在哪一帧，看这张表就行：

| 列 | 含义 |
| --- | --- |
| 协议 | 这一帧属于哪个协议（单协议导出里也是恒定的一列，几个文件可以直接拼起来） |
| 连接 | 本轮里的第几次连接，从 1 开始；重连后序号重新从 1 数，靠这一列区分 |
| 序号 | 帧里的 `sequence` |
| 到达时刻（毫秒） | 这一帧到达时距本轮开始的时间，就是图表横轴上的那个位置 |
| 帧间隔（毫秒） | 与**同一条连接**上一帧的间隔；每条连接的第一帧留空 |
| 服务端间隔（毫秒） | 服务端自己的 `server_time` 变化量。它明显大于帧间隔，说明是服务端没按时产出；两者差不多，说明帧是在链路上或客户端被拖住了 |
| 服务端时间（Unix 毫秒） | 服务端构造这一帧时的时钟，可以和服务端日志对齐 |
| 负载字节 | 帧里的 `payload_size` |
| 接收字节 | 这一帧实际占用的应用层字节 |
| 丢帧 | 这一帧之前由 `sequence` 推出的跳号数量，正常是 0 |
| 帧完整 | `payload_size` 与实际负载长度是否一致 |
| 中断 | 这一帧的帧间隔是否超过中断阈值 |

明细表有自己的表头行（汇总表是 `分组, 指标, <协议列…>`，明细表是上面这 12 列），两张表之间空一行，仍然是一个 CSV 文件、一次下载。

**明细是有限的，而且这一点写在文件里**：一轮跑几小时可以产生上百万帧，全部留在内存里会把标签页拖垮，所以每个协议只保留**最近 50000 帧**（100 毫秒间隔约 33 分钟，1 秒间隔约 5.5 小时）。元信息里有一行「逐帧明细保留」，写着每个协议的 `保留/总数`，例如 `HTTP 流 50000/1234567`——看到分母大于分子就知道这份明细只是一个窗口，而不是整轮。汇总表永远是整轮的累计值，不受这个上限影响。

> **为什么不是 `.xlsx`**：前端除了 uPlot 不带任何依赖，产物只有 ~117 KB。生成 xlsx 要再塞一个几十上百 KB 的库，而带 BOM 的 CSV 在 Excel / Numbers / Sheets 里打开的效果是一样的；明细表动辄几万行，CSV 的流式读取反而更省事。

### 总图

客户端信息栏下面是一张**总图**：把三种协议的帧间隔曲线画在**同一对坐标轴**上，一眼就能看出哪个协议和别的不一样，不用在三张卡片之间来回翻。三条线的颜色就是各自的协议色（蓝 HTTP、绿 SSE、紫 WebSocket），图例在下方。这一块**只有这一张折线图，没有表格**，也只画帧间隔——吞吐和中断点仍然只在各自的卡片里。
...
这一次请求在服务端眼里是什么样子。前端顶部的[客户端信息栏](#客户端信息栏可隐藏)用它，导出的元信息也用它。
...
```json
{
  ...
  "proto": "HTTP/1.1",
  "tls": false,
  "server_time": 1789708232746
}
```

| 字段 | 说明 |
| --- | --- |
| `ip` / `ip_source` | 要显示的地址，以及它是从哪儿来的：`cf-connecting-ip`、`true-client-ip`、`x-real-ip`、`x-forwarded-for` 或 `remote-addr` |
| `proxy` | `ip_source` 不是 `remote-addr` 时为 `true`，即这个地址是请求头声明的 |
| `remote_addr` | 套接字对端，带端口 |
| `forwarded_for` | `X-Forwarded-For` 链路，按跳数从旧到新；没发这个头时字段不出现 |
| `user_agent` / `accept_language` / `referer` | 原样回读的请求头（`referer` 缺失时不出现） |
| `host` | 请求的 `Host` |
| `proto` / `tls` / `tls_version` | 传输层：`HTTP/1.1` 还是 `HTTP/2.0`，以及是否 TLS、版本是多少 |
| `server_time` | 服务端 Unix 毫秒时间戳，字段名和帧格式一致，前端用它算时钟偏差 |

**这里不做任何外部查询**：没有地理定位、没有 IP 归属库、没有第三方接口，全部是这一次请求上直接能观测到的东西。请求头是可以伪造的——`ip_source` 就是为此存在的：它把「地址从哪来」和地址一起返回，而不是让你以为它可信。
...
│   ├── public/favicon.svg
│   ├── src/
│   │   ├── main.ts               # 三协议并发运行器 + 共享时钟 + 导出装配
│   │   ├── views.ts              # 六张表 + 每协议图表与结论
│   │   ├── client-info.ts        # 顶部客户端信息栏（IP、浏览器、时区…）
│   │   ├── export.ts             # CSV 生成与下载（汇总表 + 逐帧明细）
│   │   ├── live-text.ts          # 实时数据面板（逐帧追加正文）
│   │   ├── i18n.ts               # 中英文词典与语言切换
│   │   ├── metrics.ts            # 客户端指标采集 + 逐帧日志环形缓冲
...
| `GET /api/client` | 直连时 `ip_source=remote-addr` 且 `proxy=false`；带 `X-Forwarded-For: 203.0.113.7, 10.0.0.1` 时 `ip=203.0.113.7`、`forwarded_for` 给出完整两跳 |
| 前端导出（浏览器实测） | 汇总表 + 逐帧明细两张表同在一个 CSV，UTF-8 BOM，逐行字段数一致，`User-Agent` 里的逗号按 RFC 4180 加引号 |
