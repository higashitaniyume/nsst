...
页面最顶上是一张**总表格**，把三种协议的帧间隔读数并排放，一眼就能看出哪个协议和别的不一样，不用在三张卡片之间来回翻。列固定是三种协议，没跑的那一列显示「—」；行只有和帧间隔有关的六项：目标间隔、平均间隔、最小间隔、最大间隔、最长流间隙、中断次数。这一块**只有表格，不带图表**。
