...
打开页面后**不需要点任何按钮**，它会立刻用服务端默认参数开始测试三种协议，并且**一直跑下去，直到你点「停止全部测试」**。
...
右上角按钮切换中英文。默认跟随浏览器语言，选择记在 `localStorage`。所有文案（包括运行时的错误信息）都会切换。
...
**为什么前端不用框架？**
页面结构是固定的六张表 + 三张图，没有复杂的状态树和路由。原生 TS 让构建产物只有 ~97 KB，也没有框架升级的维护负担。
