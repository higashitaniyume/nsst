...
- **版本注入已接通** —— 版本号只在 `dotnet/Directory.Build.props` 里声明一次（当前 `0.3.1`）；`ServerConfig.ServiceVersion` 在运行时从程序集的 `AssemblyInformationalVersion` 读取，并剥掉 SDK 追加的 `+<sha>` 构建元数据。流水线用 `-p:Version=` 覆盖，**Dockerfile 就是这么做的**，C# 里没有第二份会跟它不一致的版本号。
